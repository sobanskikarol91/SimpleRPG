...
vector<Przedmiot>  Ekwipunek::spis_aktywnych_przedmiotow()
{
...
void  Ekwipunek::dodaj_przedmiot(Przedmiot nowyPrzedmiot)
{
...
Statystyki  Ekwipunek::oblicz_bonusy_przedmiotow()
{
	Statystyki suma;
...
void  Ekwipunek::zapisz_dane(string nazwa_pliku)
{
	fstream plik;
...
void  Ekwipunek::wczytaj_dane(string nazwa_pliku)
{
	fstream plik;
	string nazwa_laczona = nazwa_pliku + "_ekwipunek";
	plik.open(nazwa_laczona + ".txt", ios::in);

	przedmioty.clear();   // na wszelki wypadek czyscimy spis przedmiotow
	if (plik.good() == true)
	{
